...
class Img {
  final Image image;
  final int numOrder;
  const Img(this.image, this.numOrder);
...
Image resizeImg(File imgFile) {
  ...
  if (imageDecoded.width > imageDecoded.height) {
    ...
    if (imageResized.height < HEIGHT) {
      ...
      backgroundImage = ImageTool.copyResize(imageDecoded,
          width: WIDTH * 10, height: HEIGHT * 10);
      ImageTool.copyInto(finalImage, backgroundImage,
          dstX: (WIDTH ~/ 2 + new Random().nextDouble() * 200 - 400).toInt(),
          dstY: (HEIGHT ~/ 2 + new Random().nextDouble() * 200 - 400).toInt());
...
Image resizeImgByteData(ByteData byteData) {
  const int HEIGHT = 330;
  const int WIDTH = 440;
  const int WIDTH_THUMB = 130;
  ...
  ImageTool.Image imageDecoded = ImageTool.decodeImage(imageData);
  ImageTool.Image imageResized;
  ImageTool.Image finalImage;
  ImageTool.Image backgroundImage;
  if (imageDecoded.width > imageDecoded.height) {
